...
class Spaceship:
    ...
    def tick(self, dt):
        ...
        if 'W' in pressed_keyboards:
            self.x_speed = self.x_speed + dt * ACCELERATION * math.cos(self.rotation) #Výpočet rychlosti v smere X
            self.y_speed = self.y_speed + dt * ACCELERATION * math.sin(self.rotation) #Výpočet rychlosti v smere y
        ...
        "Ručná brzda - SHIFT"
        if 'LSHIFT' in pressed_keyboards:
            self.x_speed = 0
            self.y_speed = 0
        #Todo
...
class Game:
    ...
    def key_release(self, symbol, modifikatory):
        ...
        if symbol == key.LSHIFT:
            pressed_keyboards.discard("LSHIFT")
        pass
    """
    Start game metóda 
    """
